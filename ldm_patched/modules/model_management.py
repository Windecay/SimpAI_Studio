...
def print_vram_info_by_nvml(pos=None):
    ...
    if is_nvidia():
        ...
        pid_part = f'pid_used={pid_used_vram/1024/1024/1024:.3f}GB, ' if pid_used_vram is not None else ''
        logger.info(f'GPU memory{position}: {pid_part}used={used}, free={free}')
...
def print_memory_info(pos=None):
    ...
    if is_nvidia():
        ...
        if memory_info is not None:
            used = f'{memory_info.used/1024/1024/1024:.3f}GB'
            pid_part = f'pid_used={pid_used_vram/1024/1024/1024:.3f}GB, ' if pid_used_vram is not None else ''
            pid_and_all_used = f'{pid_part}used={used}, '
