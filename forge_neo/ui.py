...
def _build_request(
    ...
    *integrated_and_img2img,
    ...
    sketch_image = extra(1)
    sketch_foreground = extra(2)
    inpaint_image = extra(3)
    inpaint_foreground = extra(4)
    inpaint_sketch_image = extra(5)
    inpaint_sketch_foreground = extra(6)
    inpaint_upload_image = extra(7)
    inpaint_upload_mask = extra(8)
    selected_scale_tab = int(extra(9, 0) or 0)
    resize_mode = extra(10, "Crop and resize")
    resize_scale = float(extra(11, 1.0) or 1.0)
    mask_blur = extra(12, 4)
    mask_alpha = float(extra(13, 0.0) or 0.0)
    inpainting_fill = extra(14, "original")
    inpainting_mask_mode = extra(15, "Inpaint masked")
    inpaint_area = extra(16, "Only masked")
    inpaint_padding = extra(17, 32)
    soft_inpainting_enabled = bool(extra(18, False))
    soft_inpainting_schedule_bias = float(extra(19, 1.0) or 0.0)
    soft_inpainting_preservation_strength = float(extra(20, 0.5) or 0.0)
    soft_inpainting_transition_contrast_boost = float(extra(21, 4.0) or 0.0)
    soft_inpainting_mask_influence = float(extra(22, 0.0) or 0.0)
    soft_inpainting_difference_threshold = float(extra(23, 0.5) or 0.0)
    soft_inpainting_difference_contrast = float(extra(24, 2.0) or 0.0)
    batch_files = extra(25, []) or []
    batch_source_type = extra(26, "upload")
    batch_input_dir = extra(27, "")
    batch_output_dir = extra(28, "")
    batch_inpaint_mask_dir = extra(29, "")
    batch_use_png_info = extra(30, False)
    batch_png_info_props = extra(31, []) or []
    batch_png_info_dir = extra(32, "")
