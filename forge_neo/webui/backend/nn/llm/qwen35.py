...
class Qwen35VisionAttention(nn.Module):
    ...
    def forward(self, x, cu_seqlens, position_embeddings):
        ...
        for q, k, v in zip(q_splits, k_splits, v_splits):
            ...
            attend_impl = attention_pytorch if q.dtype not in (torch.float16, torch.bfloat16) and attention_function is attention_flash else attention_function
            attn_outputs.append(attend_impl(q, k, v, self.num_heads, skip_reshape=True))
