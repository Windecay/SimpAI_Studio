(function () {
    ...
    function normalizeRoleplaySpeakerMode(value) {
        const mode = String(value || '').trim().toLowerCase().replace(/[- ]/g, '_');
        if (['current', 'current_character', 'single'].includes(mode)) return 'current';
        if (['multi', 'multiple', 'sequence', 'sequential'].includes(mode)) return 'multi';
        return 'auto';
    }
    ...
    function normalizeRoleplaySession(value, conversationId = '') {
        ...
        return {
            ...
            autoplay_config: Object.assign({
                ...
                speaker_mode: normalizeRoleplaySpeakerMode(source.autoplay_config?.speaker_mode)
            }),
  ...
  ${retryable ? `<button type="button" class="describe-vlm-chat-roleplay-inline-retry" data-describe-vlm-chat-generation-run="${escapeHtml(found.actionRef)}" title="${escapeHtml(localText('Generate again', '重新生成'))}" aria-label="${escapeHtml(localText('Generate again', '重新生成'))}"><i class="fa-solid fa-rotate-right"></i></button>` : ''}
    ...
    function roleplayParticipantEntries(session) {
        ...
        const rawPresentIds = Array.isArray(scene.present_character_ids)
            ...
            : [];
        const hasExplicitRoster = rawPresentIds.length > 0;
        const presentIds = new Set(rawPresentIds);
        const playerState = normalizeRoleplayPlayerState(normalized.story_state?.player_state);
        const player = {
            id: String(normalized.persona?.id || 'player').trim() || 'player',
            entity_type: 'player',
            name: String(normalized.persona?.name || localText('You', '你')).trim() || localText('You', '你'),
            role_label: localText('Player', '玩家'),
            is_present: playerState.status === 'present',
            status_label: playerState.status === 'present'
                ? localText('Present', '在场')
                : localText('Absent', '离场'),
            status: playerState.status,
            location: '',
            condition: [],
            state_text: playerState.state_text,
            state_fields: playerState.state_fields,
            emotion: '',
            current_action: '',
            inventory: [],
            goals: [],
            avatar_asset_ids: Array.isArray(normalized.persona?.reference_asset_ids)
        ...
        const characters = roleplayCharacterEntries(normalized).map((character) => {
            ...
            const isPresent = hasExplicitRoster
                ...
                : character.id === normalized.active_character_id;
            return {
                id: character.id,
                entity_type: 'character',
                name: String(character.name || character.id || localText('Unnamed character', '未命名角色')).trim(),
                role_label: localText('Character', '角色'),
                is_present: isPresent,
                status_label: isPresent
                    ? localText('Present', '在场')
                    : localText('Absent', '离场'),
                status: isPresent ? 'present' : 'absent',
                location: String(runtime.location || '').trim(),
                condition: Array.isArray(runtime.condition) ? runtime.condition : [],
                state_text: String(runtime.state_text || '').trim(),
                state_fields: normalizeRoleplayStateFields(runtime.state_fields),
                emotion: String(runtime.emotion || '').trim(),
                current_action: String(runtime.current_action || '').trim(),
                inventory: Array.isArray(runtime.inventory) ? runtime.inventory : [],
                goals: Array.isArray(runtime.goals) ? runtime.goals : [],
                avatar_asset_ids: [
                    ...(Array.isArray(runtime.current_appearance_asset_ids) ? runtime.current_appearance_asset_ids : []),
                    String(character.avatar_asset_id || '').trim(),
                    ...(Array.isArray(character.reference_asset_ids) ? character.reference_asset_ids : [])
                ].filter(Boolean).slice(0, MAX_ROLEPLAY_REFERENCE_IMAGES + 2),
                card: character,
                runtime
            };
        });
    ...
    function roleplayParticipantAvatarSource(participant) {
        const ids = Array.isArray(participant?.avatar_asset_ids) ? participant.avatar_asset_ids : [];
        for (const assetId of ids) {
            const source = roleplayReferencePreviewUrl(assetId);
            if (source) return source;
        }
        return '';
    }

    function roleplayParticipantStatusRows(participant) {
        const rows = [];
        const add = (label, value) => {
            const text = compactRoleplayStateText(value, 180);
            if (!text || text === localText('Empty', '已清空')) return;
            rows.push(`<div class="describe-vlm-chat-roleplay-speaker-status-row"><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(text)}</dd></div>`);
        };
        add(localText('Location', '位置'), participant?.location);
        add(localText('Current action', '当前行动'), participant?.current_action);
        add(localText('Current state', '当前状态'), participant?.state_text);
        add(localText('Condition', '状况'), Array.isArray(participant?.condition) ? participant.condition.join('；') : participant?.condition);
        add(localText('Emotion', '情绪'), participant?.emotion);
        const fields = normalizeRoleplayStateFields(participant?.state_fields);
        fields.forEach((field) => add(field.label, field.value));
        add(localText('Inventory', '物品'), Array.isArray(participant?.inventory) ? participant.inventory.join('、') : participant?.inventory);
        return rows.join('');
    }
    ...
    function renderRoleplayParticipantStatusCard(participant, cardId, options = {}) {
        ...
        const avatarSource = roleplayParticipantAvatarSource(source);
        const statusClass = source.is_present ? 'is-present' : 'is-absent';
        const statusText = source.is_present
            ? localText('Present', '在场')
            : localText('Absent', '离场');
        const roleText = options.controlOnly
            ? localText('Story control', '剧情控制')
            : String(source.role_label || localText('Participant', '参与者'));
        const headline = options.controlOnly
            ? localText('Story control', '剧情控制')
            : name;
  ...
  <div class="describe-vlm-chat-roleplay-speaker-status-card" id="${escapeHtml(cardId)}" data-describe-vlm-chat-roleplay-speaker-status-card role="status">
    ...
    function positionRoleplaySpeakerStatusCard(identity, trigger = null) {
        ...
        const naturalHeight = Math.max(96, Math.min(card.scrollHeight || 300, 300));
        const naturalWidth = Math.max(card.getBoundingClientRect().width || 0, 180);
        const spaceAbove = Math.max(0, anchorRect.top - logRect.top - gap);
        const spaceBelow = Math.max(0, logRect.bottom - anchorRect.bottom - gap);
        const opensUpward = spaceBelow < naturalHeight && spaceAbove >= spaceBelow;
        const availableHeight = opensUpward ? spaceAbove : spaceBelow;
        const usableHeight = Math.max(80, Math.min(naturalHeight, availableHeight || naturalHeight));
        const spaceRight = Math.max(0, logRect.right - anchorRect.left - gap);
        const spaceLeft = Math.max(0, anchorRect.right - logRect.left - gap);
        const alignsRight = spaceRight < naturalWidth && spaceLeft > spaceRight;
    ...
    function scheduleRoleplaySpeakerCardPositionRefresh() {
        if (roleplaySpeakerCardRefreshFrame) return;
        const refresh = () => {
            roleplaySpeakerCardRefreshFrame = 0;
            const modal = document.getElementById('describe_vlm_chat_modal');
            if (!modal || modal.hidden) return;
            modal.querySelectorAll('[data-describe-vlm-chat-roleplay-speaker-identity]').forEach((identity) => {
                if (!identity.classList.contains('is-open')
                    && !identity.matches(':hover')
                    && !identity.matches(':focus-within')) return;
    ...
    function renderRoleplayParticipantChips(participants, emptyLabel) {
        const list = Array.isArray(participants) ? participants : [];
        return list.length
            ? list.map((participant) => `<span class="describe-vlm-chat-roleplay-participant-chip ${participant.is_present ? 'is-present' : 'is-absent'}" title="${escapeHtml(`${participant.name} · ${participant.status_label}`)}"><i class="fa-solid ${participant.entity_type === 'player' ? 'fa-user' : 'fa-user-astronaut'}" aria-hidden="true"></i><span>${escapeHtml(participant.name)}</span></span>`).join('')
    ...
    function renderRoleplayParticipantSummary(runtime) {
        ...
        const currentSpeaker = roleplayCurrentSpeakerForMessages(session, target?.messages);
  ...
  <span class="describe-vlm-chat-roleplay-participant-group"><b>${escapeHtml(localText('Present', '在场'))}</b><span>${renderRoleplayParticipantChips(present, localText('None', '无'))}</span></span>
  <span class="describe-vlm-chat-roleplay-participant-group is-absent"><b>${escapeHtml(localText('Absent', '离场'))}</b><span>${renderRoleplayParticipantChips(absent, localText('None', '无'))}</span></span>
  <span class="describe-vlm-chat-roleplay-current-speaker"><b>${escapeHtml(localText('Speaking', '当前发言'))}</b><span>${escapeHtml(currentSpeaker?.name || localText('Not selected', '未选择'))}</span></span>
  ${playerNotice}
</div>`;
    }
    ...
    function setRoleplayFormDraftBusy(modal, kind, busy) {
        const selectors = kind === 'character'
            ? [
                '[data-describe-vlm-chat-roleplay-character-generate]',
                '[data-describe-vlm-chat-roleplay-import-draft]'
            ]
            : [`[data-describe-vlm-chat-roleplay-draft="${kind}"]`];
        selectors.forEach((selector) => setRoleplayActionBusy(modal, selector, busy));
    }
    ...
    function chatModeLabel(mode = state.chatMode) {
        const labels = {
            chat: ['Free Chat', '自由对话'],
    ...
    function syncChatSettingsSummary(modal) {
        ...
        const runtimeValue = summary.querySelector('[data-describe-vlm-chat-runtime-summary-value]');
        const selectedVersion = resolveVlmVersion(readSelectedVlmVersion());
        const configuredMaxTokens = normalizeChatMaxTokens(state.maxTokens, 0);
        ...
        const contextValue = configuredNctx > 0
            ...
            : localText('Auto', '自动');
        const settingsText = [
            `${localText('Model', '模型')} ${currentAnswerModelLabel()}`,
            `${localText('Mode', '模式')} ${chatModeLabel(state.chatMode)}`,
            `${localText('Output', '输出')} ${outputValue}`,
            `${localText('VRAM', '显存')} ${vlmVramPolicyLabel(state.vramPolicy)}`,
            `${localText('KV', 'KV')} ${vlmKvCacheTypeLabel(state.kvCacheType)}`,
            `${localText('Context', '上下文')} ${contextValue}`,
    ...
    function syncRoleplayCharacterLibraryControls(modal, preferredId = '') {
        const select = modal?.querySelector('[data-describe-vlm-chat-roleplay-character-library-select]');
        if (!select) return;
        const selected = String(preferredId || select.value || '').trim();
    ...
    function roleplayCharacterLibraryAgentFeedback(modal, message, isError = false) {
        const feedback = modal?.querySelector('[data-roleplay-character-library-agent-feedback]');
        if (!feedback) return;
        const text = String(message || '').trim();
...
<section class="describe-vlm-chat-character-library-panel" role="dialog" aria-modal="true" aria-label="${escapeHtml(localText('Character library', '角色库'))}">
  ...
  <div class="describe-vlm-chat-character-library-body">
    ...
    <main class="describe-vlm-chat-character-library-main">
      ...
      <div class="describe-vlm-chat-character-library-editor" data-roleplay-character-library-editor hidden>
        ...
        <section class="describe-vlm-chat-character-library-visual-section">
          ...
          <div class="describe-vlm-chat-character-library-visual-top">
    ...
    async function requestRoleplayCharacterLibraryAgent(modal, mode = 'generate') {
        ...
        if (workspace.busy) {
            roleplayCharacterLibraryAgentFeedback(modal, localText('A character-agent request is already running.', '角色智能体请求正在进行中。'));
            return false;
        }
        readRoleplayCharacterLibraryForm(modal);
        const card = normalizeRoleplayCharacterLibraryCard(workspace.draft);
        ...
        const progressMessage = mode === 'visual'
        ...
        try {
            ...
            if (!response?.ok || !response.form_draft?.ok || !candidate) {
                const failureMessage = response?.ok === false
                    ? describeVlmChatFailure(response)
                    : String(response?.details || localText(
    ...
    async function deleteRoleplayCharacterLibraryWorkspaceCard(modal) {
        ...
        try {
            ...
            renderRoleplayCharacterLibraryWorkspace(modal);
            syncRoleplayCharacterLibraryControls(document.getElementById('describe_vlm_chat_modal'));
    ...
    function syncRoleplayControls(modal, runtimeOverride = null) {
        ...
        if (participantSummary) participantSummary.outerHTML = renderRoleplayParticipantSummary(runtime);
        ...
        const speakerMode = modal.querySelector('[data-describe-vlm-chat-roleplay-speaker-mode]');
        if (speakerMode) speakerMode.value = normalizeRoleplaySpeakerMode(session.autoplay_config.speaker_mode);
    ...
    function applyRoleplayForm(modal, runtime = null) {
        ...
        session.autoplay_config.speaker_mode = normalizeRoleplaySpeakerMode(
            modal.querySelector('[data-describe-vlm-chat-roleplay-speaker-mode]')?.value
        );
    ...
    async function requestRoleplayFormDraft(targetKind = 'character', runtime = currentConversationRuntime(), modal = document.getElementById('describe_vlm_chat_modal')) {
        ...
        let response;
        try {
            response = await postJson('/describe-image/vlm-chat-run', {
                request_kind: 'roleplay_form_draft',
            ...
            setRoleplayFormDraftBusy(modal, kind, false);
        }
        if (!response?.ok || !response.form_draft?.ok) {
            setConversationStatus(target, response?.ok === false
                ? describeVlmChatFailure(response)
                : localText(
    ...
    function roleplayAgentRequestRouting(session) {
        ...
        return {
            ...
            agent_routing_api_profile_version: String(routing.profiles?.api_main?.version || '').trim()
        };
    }
...
<div class="describe-vlm-chat-panel" role="dialog" aria-modal="true" aria-label="${escapeHtml(t('VLM/LLM AI chat', 'VLM/LLM AI对话'))}">
  <div class="describe-vlm-chat-head">
    ...
    <div class="describe-vlm-chat-settings-summary" data-describe-vlm-chat-settings-summary hidden aria-live="polite" aria-hidden="true">
      <span class="describe-vlm-chat-settings-summary-value" data-describe-vlm-chat-settings-summary-value></span>
      <span class="describe-vlm-chat-settings-summary-runtime"><i class="fa-solid fa-gauge-high" aria-hidden="true"></i><span data-describe-vlm-chat-runtime-summary-value>${escapeHtml(localText('Waiting for model status', '等待模型状态'))}</span></span>
    </div>
  </div>
  ...
  <div class="describe-vlm-chat-roleplay-strip" data-describe-vlm-chat-roleplay-strip hidden>
    ...
    <div class="describe-vlm-chat-roleplay-participant-summary" data-describe-vlm-chat-roleplay-participant-summary aria-live="polite"></div>
  ...
  <section class="describe-vlm-chat-roleplay-panel" data-describe-vlm-chat-roleplay-panel hidden aria-label="${escapeHtml(localText('Roleplay settings', '角色扮演设置'))}">
    ...
    <div class="describe-vlm-chat-roleplay-panel-body">
      ...
      <div class="describe-vlm-chat-roleplay-section">
        ...
        <label><span>${escapeHtml(localText('Speaking strategy', '发言策略'))}</span><select data-describe-vlm-chat-roleplay-speaker-mode><option value="auto">${escapeHtml(localText('Automatic', '自动'))}</option><option value="current">${escapeHtml(localText('Current character only', '仅当前角色'))}</option><option value="multi">${escapeHtml(localText('Multiple characters in order', '多人顺序'))}</option></select></label>
...
<div class="describe-vlm-chat-user-template-dialog" data-describe-vlm-chat-user-template-dialog-panel hidden>
  ...
  <div class="describe-vlm-chat-user-template-dialog-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(localText('User document manager', '用户项目管理'))}">
    ...
    function normalizeResponseSource(value, fallback = {}) {
        ...
        const source = container.response_source && typeof container.response_source === 'object'
            ? container.response_source
            : container;
        const fallbackSource = fallback && typeof fallback === 'object' ? fallback : {};
        const params = source.params && typeof source.params === 'object'
            ? source.params
            : container.params && typeof container.params === 'object'
                ? container.params
                : {};
        const route = source.agent_route && typeof source.agent_route === 'object'
            ? source.agent_route
            : source.roleplay_agent_route && typeof source.roleplay_agent_route === 'object'
                ? source.roleplay_agent_route
                : container.agent_route && typeof container.agent_route === 'object'
                    ? container.agent_route
                    : fallbackSource.agent_route && typeof fallbackSource.agent_route === 'object'
                        ? fallbackSource.agent_route
                        : {};
        const customApi = fallbackSource.custom_api && typeof fallbackSource.custom_api === 'object'
            ? fallbackSource.custom_api
            : {};
        const text = (raw, limit) => String(raw || '').trim().slice(0, limit);
        const version = text(
            source.version || container.version || params.version || fallbackSource.version,
            240
        );
        const provider = text(
            source.provider || container.provider || params.provider || params.custom_provider || customApi.provider,
            100
        );
        const model = text(
            source.model || container.model || params.model || params.custom_model || customApi.model,
            320
        );
        const profileId = text(
            source.profile_id
                || route.profile_id
                || params.profile_id
                || params.custom_profile_id
                || customApi.profile_id,
            160
        );
        const profileName = text(
            source.profile_name
                || route.profile_name
                || params.profile_name
                || params.custom_api_name
                || customApi.api_name
                || customApi.name,
            200
        );
        let profileType = text(
            source.profile_type || source.kind || route.profile_type || container.profile_type,
            40
        ).toLowerCase();
        if (!['api', 'local'].includes(profileType)) {
            const customSignal = version.toLowerCase() === 'custom'
                || version.toLowerCase().startsWith('custom_api:')
                || !!profileName
                || !!profileId
                || ['custom', 'openai', 'anthropic', 'google'].includes(provider.toLowerCase());
            profileType = customSignal ? 'api' : (version || model ? 'local' : '');
        }
        const kind = ['api', 'local'].includes(profileType) ? profileType : 'unknown';
        const fallbackUsed = source.fallback_used !== undefined
            ? !!source.fallback_used
            : !!route.fallback_used;
        const agentRole = text(source.agent_role || route.role, 100);
        if (!version && !provider && !model && !profileId && !profileName && !agentRole) return null;
        return {
            kind,
            profile_type: profileType,
            provider: provider || (kind === 'api' ? 'custom' : kind === 'local' ? 'local' : ''),
            model,
            version,
    ...
    function responseSourceLabel(source) {
        const item = normalizeResponseSource(source);
        if (!item) return '';
        const prefix = item.kind === 'api'
            ? t('API', 'API')
            : item.kind === 'local'
                ? t('Local', '本地')
                : t('Source', '来源');
        const identity = item.kind === 'api'
            ? (item.profile_name || item.provider || item.model || item.version)
            : (item.model || item.version || item.provider);
        const parts = [prefix];
        if (identity) parts.push(identity);
        if (item.kind === 'api' && item.model && identity !== item.model) parts.push(item.model);
        if (item.fallback_used) parts.push(t('fallback', '备用'));
        return parts.join(' · ');
    }

    function responseSourceTitle(source) {
        const item = normalizeResponseSource(source);
        if (!item) return '';
        const rows = [
            item.kind === 'api' ? t('Source: API', '来源：API') : item.kind === 'local' ? t('Source: local', '来源：本地') : t('Source: unknown', '来源：未知'),
            item.profile_name ? `${t('Profile', '配置')}：${item.profile_name}` : '',
            item.provider ? `${t('Provider', '提供方')}：${item.provider}` : '',
            item.model ? `${t('Model', '模型')}：${item.model}` : '',
            item.version ? `${t('Version', '版本')}：${item.version}` : '',
            item.fallback_used ? t('Fallback profile was used', '使用了备用配置') : ''
        ].filter(Boolean);
        return rows.join(' · ');
    }

    function responseSourceHtml(source) {
        const item = normalizeResponseSource(source);
        const label = responseSourceLabel(item);
        if (!item || !label) return '';
        const title = responseSourceTitle(item);
        return `<small class="describe-vlm-chat-response-source" title="${escapeHtml(title)}" aria-label="${escapeHtml(title)}">${escapeHtml(label)}</small>`;
    }
  ...
  <div class="describe-vlm-chat-action-buttons">
    ...
    function roleplaySpeakerLabel(message) {
        const explicitName = String(message?.roleplay_speaker_name || '').trim();
        const runtime = currentConversationRuntime();
        const session = normalizeRoleplaySession(runtime?.roleplaySession, runtime?.conversationId);
        const participant = roleplayParticipantForMessage(message, session, message?.role === 'user' ? 'user' : 'assistant');
        return String(participant?.name || explicitName || '').trim();
    }
  ...
  <div class="describe-vlm-chat-action-buttons">
    ...
    <button type="button" data-describe-vlm-chat-copy="${escapeHtml(actionRef)}" title="${escapeHtml(copyTitle)}" aria-label="${escapeHtml(copyTitle)}"><i class="fa-solid fa-copy"></i><span>${escapeHtml(copyLabel)}</span></button>
            ...
            const roleplayLabel = role === 'assistant' && isRoleplay
                ...
                : '';
            const controlLabel = role === 'user' && message.roleplay_control_only
                ? t('Story control', '剧情控制')
                : '';
            const label = roleplayLabel || controlLabel || (role === 'assistant' ? t('Assistant', '助手') : t('You', '你'));
            const roleplayIdentityHtml = roleplayParticipant
            ...
            const completion = normalizeChatCompletion(activeVariant?.completion || message.completion);
            const responseSource = role === 'assistant' && !pending
                ? responseSourceHtml(activeVariant?.response_source || message.response_source)
                : '';
  ...
  ${actionHtml}
    ...
    async function sendMessage() {
        ...
        const internalRoleplayTurn = selectedMode === 'roleplay' && options.roleplayInternalTurn === true;
        const suppressUserMessage = internalRoleplayTurn || options.suppressUserMessage === true;
        ...
        if (selectedMode === 'roleplay') {
            ...
            assistant.roleplay_speaker_name = String(
                ...
                || (speakerId === runtime.roleplaySession?.active_character_id ? runtime.roleplaySession?.character?.name : '')
                || ''
            ).trim();
        }
        ...
        if (selectedMode === 'roleplay') {
            const variant = normalizeRoleplayMessageVariant({
                ...
                roleplay_speaker_id: assistant.roleplay_speaker_id,
                roleplay_speaker_name: assistant.roleplay_speaker_name,
                roleplay_internal_turn: assistant.roleplay_internal_turn
    ...
    async function requestRoleplaySpeakerPlan(runtime, options = {}) {
        const target = runtime || currentConversationRuntime();
        const session = normalizeRoleplaySession(target.roleplaySession, target.conversationId);
        ...
        if (presentIds.length <= 1) return presentIds.length ? presentIds : (currentId ? [currentId] : []);
        ...
        updateRoleplayAutoplayState(target, {
            ...
            abort_controller: controller,
            error: ''
        }, localText('Choosing the next speakers...', '正在选择下一位发言角色……'));
        const response = await postJson('/describe-image/vlm-chat-run', {
            request_kind: 'roleplay_speaker_plan',
            roleplay_request_kind: 'speaker_plan',
            roleplay_autoplay: true,
            roleplay_speaker_mode: mode,
            roleplay_speaker_id: currentId,
            roleplay_last_speaker_id: String(options.lastSpeakerId || '').trim(),
            roleplay_autoplay_state: normalizeRoleplayAutoplayState(target.roleplayAutoplayState),
            message: localText(
                'Choose the characters who should continue the current scene.',
                '选择应该继续当前场景的角色。'
            ),
            conversation_id: target.conversationId,
            request_id: requestId,
            ...
            user_system_prompt: target.customSystemPrompt || '',
            lang: state.__lang,
            max_tokens: Math.min(800, Math.max(256, effectiveChatMaxTokens('roleplay')))
        }, { signal: controller.signal });
        const live = normalizeRoleplayAutoplayState(target.roleplayAutoplayState);
        if (live.request_id === requestId) {
            target.roleplayAutoplayState = normalizeRoleplayAutoplayState(Object.assign({}, live, {
        ...
        const plan = response.roleplay_speaker_plan && typeof response.roleplay_speaker_plan === 'object'
            ? response.roleplay_speaker_plan
            : {};
        if (plan.stop_for_player) {
            updateRoleplayAutoplayState(target, {
                phase: 'paused',
                request_id: '',
                abort_controller: null,
                reason: 'speaker_plan_stop_for_player'
            }, localText('Autoplay is waiting for player input.', '托管正在等待玩家输入。'));
            return [];
        }
    ...
    async function runRoleplayAutoplay(runtime = currentConversationRuntime(), options = {}) {
        ...
        const shouldResetCompletedTurns = !stepOnly && (
            ['idle', 'completed', 'stopped', 'error'].includes(current.phase)
            || (!continuous && current.completed_turns >= configuredTarget && current.phase !== 'running')
        );
        const startingCompletedTurns = shouldResetCompletedTurns ? 0 : current.completed_turns;
        ...
        updateRoleplayAutoplayState(target, {
            phase: 'running',
            completed_turns: startingCompletedTurns,
        ...
        while (true) {
            ...
            for (let speakerIndex = 0; speakerIndex < speakerIds.length; speakerIndex += 1) {
                ...
                response = await sendMessage({
                    ...
                    roleplaySpeakerMode: speakerMode,
                    roleplaySpeakerId: speakerId,
                    roleplayLastSpeakerId: lastSpeakerId,
                    roleplayInternalTurn: speakerIndex > 0,
                ...
                const speakerDecision = response?.roleplay_autoplay_decision;
                if (speakerDecision && ['paused', 'completed', 'error'].includes(String(speakerDecision.phase || ''))) {
                    break;
                }
            }
            ...
            const turnAccepted = !['director_failure', 'duplicate_turn'].includes(decisionReason)
                && String(decision?.phase || '') !== 'paused';
            const completed = normalizeRoleplayAutoplayState(target.roleplayAutoplayState).completed_turns + (turnAccepted ? 1 : 0);
            const pausedAfterReply = normalizeRoleplayAutoplayState(target.roleplayAutoplayState).phase === 'paused';
            updateRoleplayAutoplayState(target, {
                phase: pausedAfterReply || String(decision?.phase || '') === 'paused'
                    ? 'paused'
                    : ((!continuous && completed >= targetTurns) || stepOnly || String(decision?.phase || '') === 'completed' ? 'completed' : 'running'),
    ...
    document.addEventListener('click', (evt) => {
        ...
        const speakerTrigger = evt.target.closest?.('[data-describe-vlm-chat-roleplay-speaker-trigger]');
        if (speakerTrigger) {
            const identity = speakerTrigger.closest('[data-describe-vlm-chat-roleplay-speaker-identity]');
            if (!identity) return;
            document.querySelectorAll('[data-describe-vlm-chat-roleplay-speaker-identity].is-open').forEach((item) => {
                if (item !== identity) {
                    item.classList.remove('is-open', 'opens-upward', 'align-right');
                    const itemCard = item.querySelector('[data-describe-vlm-chat-roleplay-speaker-status-card]');
                    if (itemCard) itemCard.style.maxHeight = '';
                    item.querySelector('[data-describe-vlm-chat-roleplay-speaker-trigger]')?.setAttribute('aria-expanded', 'false');
                }
            });
            const open = !identity.classList.contains('is-open');
            identity.classList.toggle('is-open', open);
            if (open) {
                positionRoleplaySpeakerStatusCard(identity, speakerTrigger);
            } else {
                identity.classList.remove('opens-upward', 'align-right');
                const card = identity.querySelector('[data-describe-vlm-chat-roleplay-speaker-status-card]');
                if (card) card.style.maxHeight = '';
            }
            speakerTrigger.setAttribute('aria-expanded', open ? 'true' : 'false');
            evt.preventDefault();
            return;
        }
    ...
    document.addEventListener('focusin', (evt) => {
        const trigger = evt.target?.closest?.('[data-describe-vlm-chat-roleplay-speaker-trigger]');
        if (!trigger) return;
        positionRoleplaySpeakerStatusCard(trigger.closest('[data-describe-vlm-chat-roleplay-speaker-identity]'), trigger);
    });

    document.addEventListener('scroll', (evt) => {
        if (evt.target?.closest?.('[data-describe-vlm-chat-log]')) scheduleRoleplaySpeakerCardPositionRefresh();
    }, true);

    window.addEventListener('resize', scheduleRoleplaySpeakerCardPositionRefresh, { passive: true });

    document.addEventListener('keydown', (evt) => {
        const speakerTrigger = evt.target?.closest?.('[data-describe-vlm-chat-roleplay-speaker-trigger]');
        if (speakerTrigger && evt.key === 'Escape') {
            const identity = speakerTrigger.closest('[data-describe-vlm-chat-roleplay-speaker-identity]');
            identity?.classList.remove('is-open', 'opens-upward', 'align-right');
            const card = identity?.querySelector('[data-describe-vlm-chat-roleplay-speaker-status-card]');
            if (card) card.style.maxHeight = '';
