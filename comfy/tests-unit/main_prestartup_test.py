...
def _load_execute_prestartup_script(entrypoint):
    main_path = Path(__file__).resolve().parents[1] / entrypoint
...
def _load_prestartup_script_for_paths(monkeypatch, custom_nodes_paths: list[str], entrypoint):
    monkeypatch.setattr(
        folder_paths,
        "get_folder_paths",
        lambda name: list(custom_nodes_paths) if name == "custom_nodes" else [],
    )
    return _load_execute_prestartup_script(entrypoint)
...
@pytest.mark.parametrize("entrypoint", ["main.py", "main_comfyd.py"])
def test_execute_prestartup_script_handles_empty_custom_nodes_paths(monkeypatch, entrypoint):
    execute_prestartup_script = _load_prestartup_script_for_paths(monkeypatch, [], entrypoint)

    execute_prestartup_script()


@pytest.mark.parametrize("entrypoint", ["main.py", "main_comfyd.py"])
def test_execute_prestartup_script_keeps_all_timing_entries(monkeypatch, tmp_path, entrypoint):
    ...
    execute_prestartup_script = _load_prestartup_script_for_paths(monkeypatch, [str(first_custom_nodes), str(second_custom_nodes)], entrypoint)
