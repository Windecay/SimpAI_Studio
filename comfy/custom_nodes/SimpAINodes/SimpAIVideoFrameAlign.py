...
def align_video_frames(images, target_frame_count, source_fps=0.0, target_fps=0.0):
    ...
    if current_frame_count > target_frame_count:
        ...
        if source_fps > 0.0 and target_fps > 0.0 and abs(source_fps - target_fps) >= 0.01:
...
class SimpAIVideoFrameAlign(io.ComfyNode):
    ...
    @classmethod
    def execute(cls, images, target_frame_count, source_fps=0.0, target_fps=0.0):
        return io.NodeOutput(
            align_video_frames(images, target_frame_count, source_fps, target_fps)
        )
