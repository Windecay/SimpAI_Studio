...
class LLAMA_CPP_STORAGE:
    ...
    def load_model(cls, config):
        ...
        if mmproj and mmproj != "None":
            ...
            try:
                handler_parameters = inspect.signature(handler.__init__).parameters
            ...
            path_parameter = "mmproj_path" if "mmproj_path" in handler_parameters or accepts_kwargs else "clip_model_path"
            kwargs = {path_parameter: mmproj_path, "verbose": False}
...
llm_model_directory = os.path.join(folder_paths.models_dir, "LLM")

# Preserve any LLM paths loaded from extra_model_paths.yaml instead of overwriting them.
if "LLM" in folder_paths.folder_names_and_paths:
    llm_paths, registered_extensions = folder_paths.folder_names_and_paths["LLM"]
    if llm_model_directory not in llm_paths:
        llm_paths.insert(0, llm_model_directory)
    folder_paths.folder_names_and_paths["LLM"] = (llm_paths, set(registered_extensions) | set(llm_extensions))
else:
    folder_paths.folder_names_and_paths["LLM"] = ([llm_model_directory], set(llm_extensions))
preset_prompts = {
    "Empty - Nothing": "",
