...
def build_system_instructions(target_or_compiler, context=None):
    ...
    if media.get("is_masked_video_edit"):
        ...
        language_rule = (
            "Keep required H3 section names, shot markers, timestamps, and media tokens in English, "
            "but write all editable field content in "
    ...
    common = (
        ...
        "Put only spoken words inside <d>[Language] ...</d>. By default, dialogue is audio-only: "
        "once in the overall visual description, explicitly state in the output language that no dialogue subtitles, captions, "
        "or other text overlays appear on screen and dialogue is conveyed through audio only. "
        "Keep this instruction outside <d>; never turn spoken words into visible text. "
        "Only include visible text or subtitles when explicitly requested by the user. "
        "Keep shot-specific synchronized sounds in the shot narrative and summarize ambience and physical sounds in "
        "overall_soundscape without repeating dialogue; put only audience-only score in non_diegetic_music. Preserve the user's identities, actions, "
        ...
        "rebuild the shot timeline instead of copying the old timestamps. Use [Shot 1] without a timestamp for the opening "
        "at 0 seconds, and [Shot N] At MM:SS.mmm, for subsequent shots. Later starts must increase and remain inside "
        "the requested duration; develop the final shot through the target ending. Read legacy START-ENDs intervals "
        "when importing, but do not output them. Give each speaking or singing source a stable "
        "(S1), (S2), and so on. Put visible "
        "text in English double quotation marks. Inside every shot, describe the scene, camera movement, speaking actions, "
        "and synchronized sounds as natural narrative. Write one cohesive paragraph per shot, not one line per editor "
        "column. Omit stock sentences about not adding dialogue or sound effects when those fields are empty. "
        "Put the default no-subtitles constraint after the scene narrative, not before its opening action. "
        "Do not output editor column labels such as Camera:, "
        "Dialogue and visible text:, or Synchronized sound:. Do not output empty field placeholders. "
        "Keep speaker identity, voice description, and delivery outside <d>; put only the language tag and exact spoken "
        "words inside <d>. Establish each speaker from the supplied context and keep their identity across shots. "
        "For voiceover use says in an off-screen voiceover and state immediately after its <d> block that the "
        "corresponding on-screen character's lips remain closed. Do not turn ordinary dialogue into voiceover. "
        "Preserve <scenetrans>, <cutoff>, and audio continuity descriptions where applicable. "
        "Choose suitable camera movement when unspecified. Camera movement must state its type, meaningful "
    ...
    if mode == MODE_REF2VA and ("minimax_h3_avatar" in avatar_target or "minimax-h3(avatar)" in avatar_target):
        common += (
            ...
            "requested sound, not silence: the shot narrative and overall_soundscape must specify unchanged "
